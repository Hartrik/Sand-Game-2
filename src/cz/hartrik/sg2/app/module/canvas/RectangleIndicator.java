...
public class RectangleIndicator extends ANodeCursor<Rectangle> {
    ...
    protected void setSizes(double x, double y, double w, double h) {
        ...
        w = (x + w > canvas.xLocation + canvas.fxcanvas.getWidth()
                ? canvas.fxcanvas.getWidth()- x + canvas.xLocation
                : w);
        h = (y + h > canvas.yLocation + canvas.fxcanvas.getHeight()
                ? canvas.fxcanvas.getHeight()- y + canvas.yLocation
                : h);
