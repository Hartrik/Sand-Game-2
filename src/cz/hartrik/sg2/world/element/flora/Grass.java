 ...
 * @author Patrik Harag
 */
public class Grass extends Plant implements ThermalInfluencedDef, BurnableDef {
    ...
    public void doAction(int x, int y, Tools tools, World world) {
        ...
        if (findSoil == SOIL_NOT_FOUND)  // uschne - nemá půdu
            ...
            doHeigher(x, y, findSoil, tools, world);
    }
    
    protected void doHeigher(int x, int y, int findSoil,
            Tools tools, World world) {
        
        final int up = y - 1;
        if (!world.valid(x, up)) return;

        final Element element = world.get(x, up);
        if (element instanceof Air) {
            if (findSoil < maxHeight && growChance.nextBoolean()) 
                world.setAndChange(x, up, this); // vyroste
